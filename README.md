...
Портретная 2D roguelite tower-defense игра для браузера, Telegram Mini Apps и Android. Сейчас репозиторий содержит рабочий боевой прототип: неподвижный страж, автоматическая стрельба, 100 уровней арены, постоянный опыт и уровень Стража, прокачка характеристик, победа/поражение, пауза, настройки и версионированное локальное сохранение.
...
- `src/content` — типизированные определения контента и централизованный баланс арен.
...
Текущие правила боя и постоянной прогрессии описаны в [`GAME_DESIGN.md`](GAME_DESIGN.md). Формулы 100 уровней арены, скейлинг врагов, XP, штраф смерти и правила повторного фарма подробно зафиксированы в [`docs/ARENA_PROGRESSION.md`](docs/ARENA_PROGRESSION.md). Стартовые числа находятся в `src/content/firstCombat.ts`, а формулы арен — в `src/content/arenaBalance.ts`.
...
Разные типы обычных врагов, элиты, боссы, лут, дополнительные семейства оружия, временные улучшения внутри забега, backend, авторизация и платежи будут добавляться следующими вертикальными срезами.
